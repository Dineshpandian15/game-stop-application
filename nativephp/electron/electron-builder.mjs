...
// Apple Developer ID signing (production). Without these, we skip codesign/notarize for local shop builds.
...
// Azure signing configuration (Windows production)
...
if (isBuilding) {
    ...
    console.log(
        hasAppleSigning
            ? '  • macOS: Apple Developer ID signing enabled'
            : '  • macOS: local build — codesign/notarize skipped (set NATIVEPHP_APPLE_* for production)',
    );
}
...
export default {
    ...
    // Only notarize when Apple Developer credentials are configured
    ...(hasAppleSigning ? { afterSign: 'build/notarize.js' } : {}),
    win: {
    ...
    mac: {
        ...
        // Skip ad-hoc codesign for local builds — embedded PHP fails strict validation with hardened runtime
        identity: hasAppleSigning ? undefined : null,
        hardenedRuntime: hasAppleSigning,
        gatekeeperAssess: false,
        // ZIP only for local builds; DMG added when Apple signing is configured
        target: hasAppleSigning
            ? [{ target: 'dmg', arch: ['arm64', 'x64'] }, { target: 'zip', arch: ['arm64', 'x64'] }]
